            ...
            if not topic:
                if self.task_wait_time and time.time() - start > self.task_wait_time:
                    ...
                    self.log_error(fl_ctx,
                                   f"task data timeout after {self.task_wait_time} secs")
                    self.system_panic(reason='cannot get task data from T1', fl_ctx=fl_ctx)
                    break
            elif topic == Topic.END_RUN:
                self.system_panic(
                    reason='received EndRun from T1 while waiting for task data',
                    fl_ctx=fl_ctx)
                break
            elif topic == Topic.ABORT_TASK:
                self.system_panic(
                    reason='received AbortTask from T1 while waiting for task data',
                    fl_ctx=fl_ctx)
                break
