...
class AVShareableGenerator(AppDefinedShareableGenerator):
    ...
    def update_model(self, model_obj: Any, training_result: Any, meta: dict) -> Any:
        ...
        if not isinstance(training_result, AVModel):
            raise ValueError(f"training_result object must be AVModel but got {type(training_result)}")

        layers = training_result.free_layers
