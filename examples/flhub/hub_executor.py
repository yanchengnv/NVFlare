...
class HubExecutor(Executor):
    ...
    def execute(self, task_name: str, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        ...
        task_received_by_t2 = send_to_pipe(self.pipe,
                                           topic=req_topic,
                                           data=shareable,
                                           timeout=self.task_read_wait_time)
            ...
            if not topic:
                if self.task_wait_time and time.time() - start > self.task_wait_time:
                    ...
                    self.log_error(fl_ctx,
                                   f"task '{req_topic}' timeout after {self.task_wait_time} secs")
                ...
                self.log_error(fl_ctx,
                               f"received {topic} from T2 while waiting for result for {req_topic}")
                return make_reply(ReturnCode.SERVICE_UNAVAILABLE)
